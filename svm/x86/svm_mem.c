...
{
	if (pa >= phys_mem_max && svm_enabled) {
		pt_entry_t	pte;
		int		tmp = size;
		...
		*addr = (void *)pa;
		pte   = create_pte(pa, INTEL_PTE_VALID | INTEL_PTE_USER |
				       INTEL_PTE_WRITE | INTEL_PTE_NCACHE);

		while (tmp > 0) {
			if (pdir_map_page(base_pdir_pa, pa, pte))
				panic("svm_map_range: "
				      "Can't map physical memory");
