...
#define VENDOR_S3 0x5333

int s3_pci = 0;
char *s3_pci_linearbase = NULL;
...
static unsigned int s3_linear_addr = 0;
...
static void s3_getmodeinfo(int mode, vga_modeinfo * modeinfo)
{
    ...
    modeinfo->flags |= HAVE_EXT_SET | EXT_INFO_AVAILABLE;
#ifdef S3_LINEAR_SUPPORT
    modeinfo->flags |= CAPABLE_LINEAR;
    if (s3_linear_addr) {
	if (inb(0x3d5) & 0x10) {
	    modeinfo->flags |= IS_LINEAR;
	    modeinfo->aperture_size = s3_memory;
	    modeinfo->linear_aperture = s3_linear_addr;
	}
    } 
...
static int s3_saveregs(unsigned char regs[])
{
    ...
    /* if (s3_chiptype >= S3_864) { */
    if (s3_chiptype == S3_765) {/* SL: actually Trio32/64/V+ */
...
static int s3_linear(int op, int param)
{
	...
	s3_setpage(0);
	s3_setlinear((unsigned int)param >> 16);
	...
	s3_linear_addr = 0;
	return 0;
    } else if (op == LINEAR_QUERY_BASE && param == 0 && s3_pci) {
	return s3_pci_linearbase;
    } else
	return -1;
}
