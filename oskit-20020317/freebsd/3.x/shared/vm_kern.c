...
{
	return (vm_offset_t)osenv_mem_alloc(round_page(size), 0, PAGE_SIZE);
}
...
kmem_malloc(vm_map_t map, vm_size_t size, boolean_t waitflag)
{
	...
	blk = (vm_offset_t)osenv_mem_alloc(size, env_flags, PAGE_SIZE);
	if (blk == 0)
		return 0;
	...
	if (map == kmem_map &&
		...
		if (kmemlimit == 0) {
			...
			/*
			 * Reallocate the existing kmemusage array
		...
		if (newusage == NULL) {
			...
			bzero(newusage, newnpg * sizeof(struct kmemusage));
