 ...
 */
/*
 * Implements OSENV_AUTO_SIZE - so that mem_cache works better
 */
...
osenv_mem_alloc(oskit_size_t sz, osenv_memflags_t flags, unsigned align)
{
	if (flags & OSENV_AUTO_SIZE) {
                oskit_u32_t	*chunk;
		sz += sizeof(oskit_u32_t);
                flags &= ~OSENV_AUTO_SIZE;
                chunk = in_osenv_mem_alloc(sz,flags,align);
                if (chunk) {
                        *chunk++ = sz;
                }
                return chunk;
...
{
        if (!block) return;  // blech
        if (flags & OSENV_AUTO_SIZE) {
		oskit_u32_t *chunk = (oskit_u32_t *)block - 1;
                flags &= ~OSENV_AUTO_SIZE;
                in_osenv_mem_free(chunk,flags,*chunk);
        } else {
