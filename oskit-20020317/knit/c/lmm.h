...
/* These are imports to lmm modifier units */

void in_add_region(lmm_region_t *lmm_region,
                   void *addr, oskit_size_t size,
                   lmm_flags_t flags, lmm_pri_t pri);
void in_add_free(void *block, oskit_size_t size);
void in_remove_free(void *block, oskit_size_t size);
void *in_alloc(oskit_size_t size, lmm_flags_t flags);
void *in_alloc_aligned(oskit_size_t size, lmm_flags_t flags,
                       int align_bits, oskit_addr_t align_ofs);
void *in_alloc_page(lmm_flags_t flags);
void *in_alloc_gen(oskit_size_t size, lmm_flags_t flags,
                   int align_bits, oskit_addr_t align_ofs,
                   oskit_addr_t bounds_min, oskit_addr_t bounds_max);
oskit_size_t in_avail(lmm_flags_t flags);
void in_find_free(oskit_addr_t *inout_addr,
                  oskit_size_t *out_size, lmm_flags_t *out_flags);
void in_free(void *block, oskit_size_t size);
void in_free_page(void *block);
void in_dump(void);
void in_stats(void);
