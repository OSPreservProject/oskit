...
static int speedo_tx_reap(struct device *dev);
	...
	do {
		...
		if (status & 0xA400) {
			...
			if (sp->tx_full && dev->tbusy
				&& sp->dirty_tx > sp->cur_tx - TX_RING_SIZE + 2) {
...
/*
 * Returns how many descriptors were sucessfully processed
 */
static int
speedo_tx_reap(struct device *dev)
{
	struct speedo_private *sp = (struct speedo_private *)dev->priv;
